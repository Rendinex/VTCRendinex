...
contract RVTCTest is Test {
    ...
    function setUp() public {
        ...
        vm.prank(contract_owner);
        rvtc.createLicense(secondLicenseGoal);
        ...
        vm.prank(contract_owner);
        rvtc.distributeTokensForLicense(1, third_contributor, 833 * 10 ** 18);
        vm.prank(contract_owner);
        rvtc.distributeTokensForLicense(1, first_contributor, 167 * 10 ** 18);
    ...
    function testSaleOfLicense() public {
        ...
        vm.prank(first_contributor);
        rvtc.depositTokens(300 * 10 ** 18);

        vm.prank(second_contributor);
        rvtc.depositTokens(300 * 10 ** 18);

        vm.prank(third_contributor);
        rvtc.depositTokens(500 * 10 ** 18);

        // Assert the total locked tokens after deposits
        uint256 totalLocked = rvtc.totalLockedTokens();
        uint256 expectedTotalLocked = 1000 * 10 ** 18;
        ...
        assertEq(balanceFirst, 167 * 10 ** 18);
        assertEq(balanceSecond, 400 * 10 ** 18);
        assertEq(balanceThird, 433 * 10 ** 18);

        uint256 finalTotalSupply = rvtc.totalSupply();
        assertEq(finalTotalSupply, 1000 * 10 ** 18);
    }
}
